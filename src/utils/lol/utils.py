import datetime, discord, traceback

from .api import *

from utils.datautils import data, default
from utils.discordbot import get_member, emojis
  ...
  try:
    ...
    return discord.Embed(
      ...
      value = "__%s__ (lvl %s) - %s\n\n%s __%s__ | %s + %s + %s\n%s %s | %s + %s | %s/%s/%s\n\n%s\n\n%s + %s" % (
        ...
        emojis(guild).get(runes[stats["perk0"]]["name"].lower().replace(" ", "_"), ""),
        *[runes[stats["perk%d" % i]]["name"] for i in range(4)],
        emojis(guild).get(runes[stats["perkSubStyle"]]["name"].lower().replace(" ", "_"), ""),
